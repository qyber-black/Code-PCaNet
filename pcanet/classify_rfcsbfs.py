# classify_rfcsbfs.py - RFC SBFS classifier
#
...
class RFCSBFSClassifier:
  ...
  def __init__(self, model, n_estimators, max_depth,  max_features, min_samples_split, min_samples_leaf,
               ...
               features=None, data=None, classifier=None, parallel=True, verbose=0):
  ...
  def fit(self, X, Y, P, folds, parallel=True):
    ...
    if len(self.model) > 0:
      ...
      joblib.dump(self.estimator, os.path.join(self.model,"rfcsbfs_classifier.joblib"))
  ...
  def predict(self, X, Y=None, P=None, evaluate=True, parallel=True):
    ...
    if Y is not None and evaluate:
      scores = self.estimator.predict_proba(X[:,self.estimator.pca['features']])
      fpr, tpr, aucv, thres = {}, {}, {}, {}
      num_classes = np.unique(Y).shape[0]
      for l in range(num_classes):
  ...
  def load(model, verbose=0):
    fn = os.path.join(model,"rfcsbfs_classifier.joblib")
    if not os.path.exists(fn):
      raise Exception(f"Cannot load {fn}")
    if verbose > 0:
      print(f"# Loading RFCSBFSClassifier: {model}")
    classifier = joblib.load(fn)
    if not hasattr(classifier, 'pca') or \
       classifier.pca['classifier'] != 'RFCSBFSClassifier':
      raise Exception("Not a RFCSBFSClassifier")
    return RFCSBFSClassifier(model=model, n_estimators=classifier.pca['n_estimators'],
