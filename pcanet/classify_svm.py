# classify_svm.py - SVM classifier
#
...
class SVMClassifier:
  ...
  def __init__(self, model, kernel, C, inp_features, size, channels, standardise, normalise,
               std_scaler=None, norm_scaler=None, data=None, classifier=None, verbose=0):
  ...
  def fit(self, X, Y, P, folds, parallel=True):
    ...
    if self.verbose > 0:
      print("# Cross validation for SVM classifier training: {num_classes} classes")
    ...
    if len(self.model) > 0:
      ...
      joblib.dump(self.estimator, os.path.join(self.model,"svm_classifier.joblib"))
  ...
  def load(model, verbose=0):
    fn = os.path.join(model,"svm_classifier.joblib")
    if not os.path.exists(fn):
      raise Exception(f"Cannot load {fn}")
    if verbose > 0:
      print(f"# Loading SVMClassifier: {model}")
    classifier = joblib.load(fn)
    if not hasattr(classifier, 'pca') or \
       classifier.pca['classifier'] != 'SVMClassifier':
      raise Exception("Not a SVMClassifier")
    return SVMClassifier(model=model, kernel=classifier.pca['kernel'],
                         C=classifier.pca['C'],
